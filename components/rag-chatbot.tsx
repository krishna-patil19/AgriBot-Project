            ...
            <main className="flex-1 flex flex-col min-w-0">
                ...
                <header className="flex-shrink-0 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            {!sidebarOpen && (
                                <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 mr-1">
                                    <ChevronLeft className="w-4 h-4 rotate-180" />
                                </button>
                            )}
                            {selectedAgent ? (
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${getAgent(selectedAgent).color} flex items-center justify-center shadow-lg shadow-${getAgent(selectedAgent).color.split("-")[1]}-500/30 transform transition-transform hover:scale-105`}>
                                        <span className="text-2xl drop-shadow-sm">{getAgent(selectedAgent).icon}</span>
                                    </div>
                                    <div>
                                        <h3 className={`text-lg font-black ${getAgent(selectedAgent).textColor} dark:text-white tracking-tight`}>{getAgent(selectedAgent).name}</h3>
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t(getAgent(selectedAgent).specialtyKey as any)}</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-bold text-slate-900 dark:text-white">{t("autoRoutingMode")}</h3>
                                        <p className="text-[10px] text-slate-500">{t("autoRoutingDesc")}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="relative group">
                                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors">
                            ...
                            <Button variant="ghost" size="sm" onClick={clearChat} className="h-8 w-8 p-0 text-slate-400 hover:text-red-500">
                    ...
                    <div className="max-w-3xl mx-auto px-6 py-8 space-y-5">
                        ...
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-20">
                                <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-emerald-100 to-green-50 flex items-center justify-center mb-6 shadow-sm">
                                    <Sparkles className="w-10 h-10 text-emerald-600" />
                                </div>
                                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">{t("personalAssistantTitle")}</h3>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full max-w-2xl">
                                    {[
                                        ...
                                        <button
                                            ...
                                            className="flex flex-col items-center gap-3 p-5 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-3xl hover:border-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:shadow-xl hover:shadow-emerald-500/10 transition-all group"
                                        >
                                            <span className="text-4xl group-hover:scale-110 group-hover:-translate-y-1 transition-transform duration-300 drop-shadow-sm">{s.emoji}</span>
                                            <span className="text-sm text-slate-700 dark:text-slate-300 font-bold text-center group-hover:text-emerald-700">{s.text}</span>
                                        </button>
                                    ))}
                                </div>
                                <div className="flex items-center gap-6 mt-8">
                                    <button onClick={() => imageInputRef.current?.click()} className="flex flex-col items-center gap-1.5 group">
                                        <div className="w-12 h-12 rounded-2xl bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center group-hover:bg-rose-100 transition-colors">
                                            <ImageIcon className="w-5 h-5 text-rose-500" />
                                        </div>
                                        <span className="text-[10px] text-slate-500 font-medium">{t("uploadImage")}</span>
                                    </button>
                                    <button onClick={() => audioInputRef.current?.click()} className="flex flex-col items-center gap-1.5 group">
                                        <div className="w-12 h-12 rounded-2xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center group-hover:bg-violet-100 transition-colors">
                                            <Music className="w-5 h-5 text-violet-500" />
                                        </div>
                                        <span className="text-[10px] text-slate-500 font-medium">{t("uploadAudio")}</span>
                                    </button>
                                    <button onClick={() => datasetInputRef.current?.click()} className="flex flex-col items-center gap-1.5 group">
                                        <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center group-hover:bg-emerald-100 transition-colors">
                                            <Upload className="w-5 h-5 text-emerald-500" />
                                        </div>
                                        <span className="text-[10px] text-slate-500 font-medium">{t("knowledgeBaseTab")}</span>
                                    </button>
                                </div>
                            </div>
                        )}
